...
- Under WSL2, only opaque SSH-target trust may persist. Host-command and project-script execution is one-shot because it leaves the Linux sandbox and could otherwise reach Windows-mounted disks. Never use it for /mnt, PowerShell, cmd.exe, wsl.exe, explorer.exe, or another Windows executable; those remain direct human authority.
