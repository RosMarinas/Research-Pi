...
- When justified work needs an outside file, exact SSH target, or an existing project script that uses host facilities, use `host_capability`. State the exact target and operation. The user may approve it once or for this Pi session; do not treat one target grant as authority over another. Private keys, `.env`, cloud credentials, and keychains cannot become model-readable. SSH credentials remain opaque inside the broker.
- Project-script grants bind the canonical in-project path, exact SHA-256, and exact argv. If code or arguments change, request a new grant. Codex advisor may use external-read only; executor may use the SSH and script grants visible in its delegation context.
- A shell sandbox denial means the task crossed the project boundary. Do not work around it with symlinks, subprocesses, environment variables, temp paths, proxy commands, copied credentials, or another delegation. Request the narrow host capability first; if it cannot express the operation, provide a copy-paste `!command`. `!` and `!!` are human authority channels and are not agent permissions.
