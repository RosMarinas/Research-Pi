...
- Give consecutive work on one research subtask a short, stable `mission` label and use `reuse=auto`; this deterministically resumes the latest Codex thread with the same workspace, mission, and mode. Use a fresh mission for an independent critique, a different research route, a different workspace, or substantially stale assumptions. Never reuse context merely because two tasks share a repository.
...
- Use `steer` to send a material correction or new evidence into the active Codex turn. Do not use it for routine progress checks. If Pi restarts, reattach to jobs belonging to this exact session rather than creating replacements. Use the `missions` action when the relevant job chain is unclear.
...
- Do not launch duplicate Codex jobs merely because a background job is still running. Use status/result/respond/steer/resume/cancel on the existing job. Codex jobs are bound to one workspace and cannot be managed or resumed from another workspace. Do not delegate recursively or in parallel unless the task genuinely benefits and the workspaces or effects cannot conflict.
