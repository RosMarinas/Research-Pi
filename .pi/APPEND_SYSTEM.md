...
- Under WSL2, only opaque SSH-target trust may persist. Host commands and project scripts are one-shot and must never access `/mnt` or launch PowerShell, `cmd.exe`, `wsl.exe`, `explorer.exe`, or another Windows executable; those remain direct human authority.
