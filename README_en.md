...
Then open Ubuntu and install the Linux version of Node.js `>=22.19` and the basic dependencies:
...
Choose the branch that fits your setup:

| Branch | Best for | Main difference |
|---|---|---|
| `main` | Projects kept strictly under WSL `/home/...`, with the same behavior as macOS/Linux | Receives updates first; host commands can receive persistent project approval, without additional Windows interop checks |
| `windows-research-pi` | Greater agent autonomy with stricter isolation from the Windows host | WSL2-only preview; rejects `/mnt/*` workspaces, blocks `.exe`/PowerShell/cmd/WSL interop, permits only one-shot host-command/project-script approval, and refuses to start without seccomp |

For ordinary WSL2 use, install `main`:
...
For stricter Windows host isolation, install the Windows branch:
...
Whichever branch you choose, keep Research Pi and research projects in the WSL filesystem, such as `~/research/...`, rather than `/mnt/c` or `/mnt/d`. `windows-research-pi` is not a more feature-rich Windows edition: it periodically follows `main` for Runtime, ProjectView, and model features while tightening the execution and file boundaries between WSL and the Windows host. See the [Windows / WSL2 guide](docs/windows-wsl-guide.md) for installation, checks, and limitations.
...
| Experiment Records | Records questions, interventions, observations, and validity judgments; supports research transitions and narrow state amendments |
...
<research-project>/.pi/      project experiment records and checkpoints
```
...
The shortest Windows migration path is WSL2, not interpreting the existing Bash harness in PowerShell. `main` can run in a normal WSL2 Linux environment; use `windows-research-pi` when Windows system-drive access and host interop must also be blocked. Keep research projects under WSL's own `/home/...` filesystem in either case.

At startup, the Windows branch additionally requires WSL2, bubblewrap, socat, ripgrep, `fd`/`fdfind`, and a working seccomp helper, and runs a side-effect-free Windows host-interop probe. It fails closed if seccomp is missing, the project is under `/mnt`, or `cmd.exe` can launch inside the sandbox. Project-approved SSH targets can retain authorization; generic host commands and project scripts require one-shot approval. See the [Windows / WSL2 guide](docs/windows-wsl-guide.md) for setup and boundary details.
...
- [Windows / WSL2 installation and security boundaries](docs/windows-wsl-guide.md)
