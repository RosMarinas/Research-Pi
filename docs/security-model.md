...
For a Git-backed research project, Research Pi locally excludes `/.pi/` through `.git/info/exclude`; it does not edit the shared `.gitignore`. Experiment records remain local and must not be committed. Codex retention runs at most once per day and archives only completed/failed/cancelled jobs that satisfy both configured age and count thresholds. The exact structured job/result remains in `codex/archive/jobs.jsonl`; active, input-required and `outcome_unknown` jobs plus Codex thread/context SQLite are never removed by this policy.
