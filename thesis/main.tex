...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.8cm}YY}
    ...
    \term{Project State}\par{\footnotesize via Research Compaction}
    & 在阶段检查点综合 research question、claim、竞争假设、观察、决策、混淆因素和下一实验；按 revision 持久化，amendment/transition 做显式更新
    & \term{Project 级持久状态源}。回答“当前依据什么相信什么”，跨 Session 共享且带 provenance \\
    \addlinespace[2pt]\cmidrule(lr){1-3}\addlinespace[2pt]
    \memtag{M4}\quad\term{调用视图}\par\smallskip
    \term{ProjectView}
    & 每次从 Project State、最新 evidence/transition、task handoff、Git 与 Runtime 状态重新渲染限长 snapshot/delta
    & \term{可重建的模型上下文投影}。回答“这次调用需要看到什么”，带 freshness，但不是新的状态源 \\
    \bottomrule
  \end{tabularx}
\end{table}

四层并不互相替代：Research Memory 找回精确来源，Experiment Ledger 保存证据，Compact 形成 Project State，ProjectView 再与最新 delta 共同生成 Model Context。\term{M3 是状态源，M4 是状态视图}：M3 持久、低频并承载 Project 级综合；M4 按 Session/调用重建、受上下文预算约束，随 freshness 和 live delta 变化。新 evidence 可以先出现在 M4 中，但在 compact/amend/transition 前仍标记为“尚未综合”，不能反向覆盖 M3。普通 compact 回答“此前对话说了什么”；Research Compaction 则回答“现在依据什么相信什么、还不知道什么、下一实验要区分什么”。
...
ProjectView 使用 append-only snapshot/delta：历史请求尽量保持为共同前缀，新 evidence、handoff 和 freshness 变化集中在尾部，从而兼顾状态及时性与 DeepSeek KV-cache locality。Research compact 只在 agent 和工具链 settled 后触发；soft 阈值为 272K、hard 阈值为 384K，三档 recent tail 为 32K、40K 和 48K。缓存收益最终以 provider 的 cache-read tokens 判断，不由 Harness 自己宣布。
...
\]
其中 $P$ 是 Harness 前缀，$H_{t-1}$ 是进入本轮前的持久 Session 历史，$U_t$ 是当前用户请求，$W_{t,k}$ 是本轮已发生的 assistant/tool 消息，$\Delta V_{t,k}$ 是同一轮中新 evidence 或 Runtime 变化产生的临时 ProjectView delta。
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[stable, below=2.5mm of p] (h) {%
      ...
      \footnotesize Research compact、保留的 recent tail、既往对话与工具结果，以及进入本轮时的 ProjectView snapshot 和已接收 mailbox。};
    ...
    \node[delta, below=2.5mm of w] (d) {%
      \term{$\Delta V_{t,k}$\quad ProjectView 实时增量}\hfill
      \textcolor{RPGray}{来源：语义状态变化；更新：必要时追加}\\[-1pt]
      \footnotesize 尚未进入稳定 snapshot 的最新 evidence、transition、handoff、Git/Runtime 与 mailbox freshness；无实质变化时不重复注入。};
  ...
  \end{tikzpicture}
  \caption{一次 Research-Pi 模型调用的纵向上下文栈。上方 $P$ 与 $H_{t-1}$ 构成跨轮稳定前缀；下方 $U_t$、$W_{t,k}$ 与 $\Delta V_{t,k}$ 按事件追加。ProjectView 的稳定 snapshot 位于 $H_{t-1}$，尚未综合的新变化位于末尾 delta。只要已有字节不变，provider 就能从左逻辑前缀（图中自上而下）复用 KV cache；compact、模型/工具定义变化或从历史中移除消息会建立新的缓存边界。}
